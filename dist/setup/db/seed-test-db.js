...
const seed = async () => {
  try {
    ...
    // Users
    const adminPassword = await (0, _users.hashPassword)(process.env.ADMIN_PASSWORD);
    const yukiPassword = await (0, _users.hashPassword)(process.env.YUKI_PASSWORD);
    const pierrePassword = await (0, _users.hashPassword)(process.env.PIERRE_PASSWORD);
    ...
    const {
      rows: users
    } = await pool.query(userInsertQuery, [adminPassword, yukiPassword, pierrePassword]);
