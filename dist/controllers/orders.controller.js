...
exports.updateOrderStatus = exports.getOrderForUser = exports.getAllUserOrders = exports.getAllOrders = exports.deleteUserOrder = exports.createOrderFromCart = void 0;
...
const getAllUserOrders = async (req, res) => {
  try {
    const result = await _db.default.query("SELECT * FROM orders WHERE user_id = $1", [req.user.id]);
    res.status(200).json(result.rows);
  } catch (error) {
    console.log("Error fetching orders: ", error);
    res.status(500).json({
...
const getOrderForUser = async (req, res) => {
  try {
    ...
    const doesOrderExist = await (0, _orders.doesOrderExistForUser)(req.user.id, orderId);
    if (!doesOrderExist) {
      return res.status(404).json("Order does not exist for user.");
    }
    const result = await _db.default.query("SELECT * FROM order_items WHERE order_id = $1", [orderId]);
    res.status(200).json(result.rows);
  } catch (error) {
...
const createOrderFromCart = async (req, res) => {
  ...
  try {
    ...
    const cartItemRes = await client.query("SELECT ci.product_id, ci.quantity, p.price FROM cart_items ci JOIN products p ON ci.product_id = p.id WHERE ci.cart_id = (SELECT id FROM carts WHERE user_id = $1)", [req.user.id]);
    ...
    const orderRes = await client.query("INSERT INTO orders (user_id, total_price, status) VALUES ($1, $2, $3) RETURNING id", [req.user.id, totalPrice, "PENDING"]);
    ...
    await client.query("DELETE FROM cart_items WHERE cart_id = $1", [req.user.cart_id]);
...
const updateOrderStatus = async (req, res) => {
  try {
    ...
    const validAdminStatuses = ["PROCESSING", "READY FOR PICK-UP", "COMPLETE", "CANCELLED"];
    if (isNaN(orderId)) {
      return res.status(400).json({
    ...
    const result = await _db.default.query("UPDATE orders SET status = $1 WHERE id = $2 AND user_id = $3 RETURNING *", [newStatus, orderId, userId]);
    res.status(200).json({
...
const deleteUserOrder = async (req, res) => {
  try {
    ...
    const result = await _db.default.query("DELETE FROM orders WHERE id = $1 RETURNING *", [orderId]);
