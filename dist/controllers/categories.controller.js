...
exports.updateCategory = exports.getProdsByCategory = exports.getAllCategories = exports.deleteCategory = exports.createNewCategory = void 0;
...
const getProdsByCategory = async (req, res) => {
  try {
    ...
    const result = await _db.default.query("SELECT name, description, price, stock, category_id FROM products WHERE category_id = $1", [categoryId]);
...
const createNewCategory = async (req, res) => {
  try {
    ...
    const result = await _db.default.query("INSERT INTO categories (name) VALUES ($1) RETURNING *", [newCategory]);
    res.status(201).json({
...
const updateCategory = async (req, res) => {
  try {
    ...
    const result = await _db.default.query("UPDATE categories SET name = $1 WHERE id = $2 RETURNING *", [categoryUpdate, categoryId]);
    if (result.rows.length == 0) {
      return res.status(404).json({
        error: "Category not found."
      });
...
const deleteCategory = async (req, res) => {
  try {
    ...
    const result = await _db.default.query("DELETE FROM categories WHERE id = $1 RETURNING *", [categoryId]);
    if (result.rows.length == 0) {
      return res.status(404).json({
        error: "Category not found."
      });
